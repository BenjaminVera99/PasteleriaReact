...
export default function Admin() {
  ...
  // ✅ Comprobar token y rol una sola vez
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        `Error al ${editingId ? 'actualizar' : 'crear'} el producto. Revisa la consola para más detalles del Backend.`
      )
    }
  }
  ...
  // Mientras no terminamos de chequear auth, no renderizamos
  if (!checkedAuth) {
    return null
  }

  // Si tiene sesión pero NO es admin, mostramos mensaje (no lo mandamos al Home)
  if (!isAuthorized) {
    return (
      <>
        <Navbar />
    ...
    <>
      ...
      <main className="container py-4">
        ...
        <form className="card pastel-card p-3 mb-3" onSubmit={handleSubmit}>
          <h2 className="h5 text-choco mb-3">
          ...
          <div className="row g-2">
            ...
            <div className="col-md-2 d-flex align-items-center gap-2">
              ...
              />
              <label htmlFor="onSale" className="m-0">
                Oferta
              </label>
            </div>
          </div>
